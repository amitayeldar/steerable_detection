...
num_noise_patch_sim = 50  # number of noise patches to simulate the noise from
...
for mrc_file in files_micro:
    ...
    a = np.cov(noise_vec_sim)
    rell_err = np.linalg.norm(a - covariance_matrix) / np.linalg.norm(covariance_matrix)
    print(rell_err)
    # Simulate S_z from noise patches
    device = "mps" if torch.backends.mps.is_available() else "cuda" if torch.cuda.is_available() else "cpu"

    S_z_sci = projected_noise_simulation_from_noise_patches_torch(noise_vec_sim-np.mean(noise_vec_sim), sorted_basis_images,device=device)
    z_max = np.max(S_z_sci, axis=(0, 1)).reshape(-1, 1)
    # Step 4: Compute Peaks
    centerd_Y = Y - np.mean(noise_patches_sim)
    Y_peaks, Y_peaks_loc, S = peak_algorithm_cont_mask_torch(centerd_Y, sorted_basis_images, np.floor(sideLengthAlgorithmL),contamination_mask_binary,
        obj_sz_down_scaled=obj_sz_down_scaled, debug=False, device = device)
    ...
    M_L = (Y.shape[0] * Y.shape[1]) / ((S_z_sci.shape[0] / (2*np.sqrt(2))) ** 2)
